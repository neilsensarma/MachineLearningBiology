...
def pca(X, n_pcs = 2):
    ...
    egn_vals = vals[index]
    egn_vectors = vecs[:, index]
    egn_vectors_subset = egn_vectors[:, 0:n_pcs]
    return -np.dot(egn_vectors_subset.transpose(), X.transpose()).transpose()
...
def pca_sklearn(X, n_pcs = 2):
    ...
    # implement your code here
    return PCA(n_components=n_pcs).fit(X).transform(X)
...
# read data matrix
expr_ctrl = pd.read_csv("D:\\GaTech\\Spring Sem\\CX 4803\\HW 2\\hw2_coding\\hw2_coding\\counts_PBMC.csv", sep = ",", index_col = 0).values
anno_ctrl = pd.read_csv("D:\\GaTech\\Spring Sem\\CX 4803\\HW 2\\hw2_coding\\hw2_coding\\celltypes_PBMC.txt", sep = "\t", header = None)
X_pca = pca(expr_ctrl)
